#!/usr/bin/env node
/**
 * CCView MCP Server — Exposed via Streamable HTTP
 *
 * Auth model: Client sends their own CCView API key as Bearer token.
...
import { CCViewClient } from './api-client.js';
...
app.get('/', (_req, res) => {
  ...
  res.json({
    ...
    version: '1.0.0',
    description:
      'Access Canton Network explorer data — transfers, governance, validators, rewards, parties, mining rounds — through MCP tools.',
    mcpEndpoint: '/mcp',
    ...
    auth: {
      type: 'dual-mode',
      description:
        'Pass your CCView API key as the Bearer token. No credentials are stored on this server.',
      header: 'Authorization: Bearer <your-ccview-api-key>',
      howToGetKey:
        'Obtain an API key from ccview.io',
    },
    configTemplate: {
      mcpServers: {
        ccview: {
          ...
          headers: {
            Authorization: 'Bearer <your-ccview-api-key>',
          },
        },
      },
    },
    links: {
      health: '/health',
      documentation: 'https://financemcps.agenticledger.ai/ccview/',
    },
  });
});
...
<head>
  ...
  <title>CCView MCP Server — AgenticLedger</title>
...
<body>
  ...
  <div class="container">
    <h1>CCView MCP Server</h1>
    <p class="subtitle">Access Canton Network explorer data — transfers, governance, validators, rewards, parties, mining rounds — through MCP tools.</p>

    <div class="card">
      <h2>How to get your CCView API key</h2>
      <ol class="steps">
        <li>Visit <a href="https://ccview.io" target="_blank" rel="noopener">ccview.io</a>.</li>
    ...
    <div class="card">
      ...
      <label for="apiKey">CCView API Key</label>
...
function createMCPServer(client: CCViewClient): Server {
  const server = new Server(
    { name: 'ccview-mcp-server', version: '1.0.0' },
    { capabilities: { tools: {} } }
  );
...
app.post('/mcp', async (req, res) => {
  ...
  const client = new CCViewClient(apiKey);
...
<head>
  ...
  <title>Authorize \u2014 CCView MCP</title>
...
<body>
  <div class="card">
    <div class="header"><img src="/static/logo.png" alt="AgenticLedger"><span>CCView MCP</span></div>
    <div class="consent-msg">An application wants to connect to <strong>CCView MCP Server</strong> on your behalf. Enter your API key to authorize access.</div>
...
app.listen(PORT, () => {
  console.log(`CCView MCP HTTP Server running on port ${PORT}`);
  console.log(`  MCP endpoint:   http://localhost:${PORT}/mcp`);
  console.log(`  Health check:   http://localhost:${PORT}/health`);
  console.log(`  Tools:          ${tools.length}`);
  console.log(`  Transport:      Streamable HTTP`);
  console.log(`  Auth:           Bearer passthrough (client provides CCView API key)`);
});
// OAuth upgrade 2026-03-19T00:52:14Z
